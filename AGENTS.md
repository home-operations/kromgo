...
- **Idempotent.** Reconcilers, code generators (`mise run generate`), and CLI
  ...
  affects correctness, only latency. This repo's gallery/chart generation
  (`mise run gallery`) should be no different: re-running it with unchanged
...
- **DRY and minimal, without premature abstraction.** Three similar call
  ...
  unprompted.
- **Unit tested**, table-driven via `t.Run` subtests. This repo uses
  `testify` (`assert`/`require`) throughout, including `test/integration`
  and `test/e2e`; match that rather than introducing bare stdlib assertions
  in new test files. `go test -race` is the floor (`mise run test`); don't
  submit goroutine-touching code you haven't run under `-race`.
- **`log/slog`**, JSON handler to stdout by default (a text-format escape
  ...
  library; this repo's `internal/logging` package wraps `slog` setup, use
  it rather than constructing a handler ad hoc. Call `slog.SetDefault` once
  in `main`, then use package-level `slog.Info`/`slog.Error`/etc.
...
- **`github.com/spf13/pflag`, only when the app has a real CLI surface**:
  ...
  serve." This repo is a server, not flag-driven; wire pflag through
  `github.com/spf13/cobra` (see `chaski`, `flate`, `ocharted`) only if a
  real CLI surface is added, rather than a bare `pflag.FlagSet`.
...
`konflate`, which is why it isn't under `internal/`; this repo has no such
export surface). Keep `main.go` to wiring: parse config, build the logger,
construct dependencies, run, translate the top-level error into an exit
code. Business logic belongs in `internal/<package>`, not in `main`.
...
This repo also has `mise run assets`, `generate`/`generate-check`,
`test-e2e`, `test-integration`, `gallery`, and `helm-*`; check `mise tasks`
...
genuinely requires cgo (this repo renders charts/images via
`golang/freetype`/`golang.org/x/image` and `go-analyze/charts`, which are
pure Go; keep it that way rather than pulling in a cgo-based graphics
dependency). Expose Prometheus metrics via `github.com/prometheus/client_golang`
on a separate port from the main listener.
